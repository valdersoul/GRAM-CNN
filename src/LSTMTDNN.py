...
class LSTMTDNN(Model):
	...
	def __init__(self,
		...
		if self.use_pts:
			...
			#self.total_emb_size += self.pt_emb_size
	...
	def __build(self, pretrained_word_embedding):
		with tf.variable_scope("LSTMTDNN"):
			...
			# use batch_norm
			#bn = batch_norm()
			#batch_norm_out = bn(tf.expand_dims(tf.expand_dims(combined_emb, 1), 1), train = not self.forward_only)
			combined_emb = tf.reshape(combined_emb, [-1,self.total_emb_size])
			combined_emb = tf.expand_dims(combined_emb, 0)
			combined_emb = tf.nn.dropout(combined_emb, keep_prob = 1- self.drop_rate)
			#combined_emb = tf.expand_dims(combined_emb, 0)

			#combined_emb_dp = tf.nn.dropout(combined_emb, 1 - self.drop_rate)
			...
			if not self.padding:
				...
				#self.loss = tf.get_variable(float32)
			else:
				# use a 1x1 CNN to deduction
				# combined_emb = tf.reshape(combined_emb, [1, -1, 1, self.total_emb_size])
				# combined_emb = conv2d(combined_emb, 2 * self.w_emb_size, 1, 1, name = "reduction")
				# combined_emb = tf.reshape(combined_emb, [-1, 2 * self.w_emb_size])
				# combined_emb = tf.nn.dropout(combined_emb, keep_prob = 1- self.drop_rate)
				# linear0 = tf.expand_dims(combined_emb, 0)
				...
				# w_1 = tf.get_variable("w_1", [cnn_output.get_shape()[1], 400])
				# b_1 = tf.get_variable("b_1", [400])
				# linear1 = tf.matmul(cnn_output, w_1) + b_1

				# w_2 = tf.get_variable("w_2", [400, 200])
				# b_2 = tf.get_variable("b_2", [200])
				# linear2 = tf.matmul(tf.tanh(linear1), w_2) + b_2

				# w_3 = tf.get_variable("w_3", [200, self.num_classes])
				# b_3 = tf.get_variable("b_3", [self.num_classes])
				# self.logits = tf.matmul(tf.tanh(linear2), w_3) + b_3
			if not self.crf:
				...
				#real_path_score = tf.reduce_sum( tf.py_func(index_along_every_row, [self.logits, self.target], [tf.float32])[0] )

				self.loss = tf.reduce_mean(-log_likelihood)
				#self.loss =  tf.reduce_mean(real_path_score - log_likelihood)
